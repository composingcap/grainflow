...
public:
	...
	void ParseVizInfo(dict& vizDict)
	{
		...
		for (int i = 0; i < vizDict.keyCount(); i++)
		{
			...
			{
				dict speakers = (atom)vizDict[name];
				if (!speakers.valid()) continue;
				set_speaker_positions(speakers);
			}
		};
	}
	...
	void set_speaker_positions(dict& speakers)
	{
